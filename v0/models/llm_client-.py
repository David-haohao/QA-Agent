...
class LLMClient:
    ...
    def chat(self, messages: list, stream: bool = False, enable_thinking: bool = True):
        """
        通用对话接口（默认开启Qwen思考模式，关闭流式输出）
        messages: [{"role": "system/user/assistant", "content": "..."}]
        stream: 是否使用流式输出（注意：stream=True 与 enable_thinking=True 不可同时使用）
        enable_thinking: 是否开启Qwen思考模式（默认True）
        ...
        kwargs = {
            ...
            "extra_body": {"enable_thinking": enable_thinking},  # Qwen思考模式
        }
